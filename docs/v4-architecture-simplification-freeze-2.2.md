# V4 Architecture Simplification Freeze 2.2
...
Review of Freeze 2.1: **C. FAIL — one or more material gaps remain** (0 BLOCKERS, 1 HIGH, 1 MEDIUM)
Historical maximal architecture: [`docs/v4-architecture-freeze-1.0.md`](v4-architecture-freeze-1.0.md)
Historical reviewed freezes (unchanged):
[`docs/v4-architecture-simplification-freeze-2.0.md`](v4-architecture-simplification-freeze-2.0.md),
[`docs/v4-architecture-simplification-freeze-2.1.md`](v4-architecture-simplification-freeze-2.1.md)
Freeze 2.1 commit / reviewed HEAD: `7765d3ff638b58927a3a2d735764eeb31ebd35db`
Historical implementation subject HEAD: `f14720485a54c85864e833573711918083af9081`

This document does **not** overwrite Freeze 1.0, Freeze 2.0, or Freeze 2.1.

- Freeze 1.0 remains the historical **maximal** V4 architecture.
- Freeze 2.0 remains the first reviewed simplification freeze (not implementation-safe).
- Freeze 2.1 remains the second reviewed simplification freeze (not implementation-safe).
- Freeze 2.2 **supersedes Freeze 2.1** as the implementation target.
- Freeze 2.2 otherwise preserves Freeze 2.1 semantics. Only the two named corrections below are new.
...
Implementation requires Project Controller approval after **one final two-delta verification**.

---

## Review result and named deltas

Targeted correctness review of Freeze 2.0 returned **C. FAIL** (Blocker-1, Blocker-2). Freeze 2.1 recorded those corrections. They remain in force and are not reopened.

Final delta review of Freeze 2.1 returned **C. FAIL — one or more material gaps remain**:

- **0 BLOCKERS**
- **1 HIGH** — Phase 1A PREPARE loser could read as if a generic “if expired → EXPIRED” step ran before classifying legacy unresolved states, which could release occupancy while old execution authority still exists.
- **1 MEDIUM** — Freeze 2.1 HALTed contradictory **terminal** pairs with a committed leave result, but did not explicitly HALT `AWAITING_CONFIRMATION` or `CONFIRMED` plus a committed source-linked or same-key leave result.

These are not a reversal of the simplified core. The following Freeze 2.1 decisions stay unchanged and must not be reopened:

- same-Postgres atomic execution
- LangGraph / checkpoint / outbox / ledger / lease / fencing removal
- persisted `EXECUTING`, `UNKNOWN_OUTCOME`, and `RECONCILING` removal from the **final** target
- employee advisory xact lock
- canonical `action_revisions` mutex
- final lock ordering
- TTL target execution model
- `IntegrityError` transaction model
- adoption equivalence model
- dynamic balance / overlap semantics
- transient retry model
- legacy schema compatibility strategy
- Attack-7 replacement design
- phase ordering except the two clarifications in this document

### HIGH — Phase 1A PREPARE must not TTL-normalize legacy unresolved states

During Phase 1A, while legacy executor / old worker authority may still exist, distinguish:

| Class | States | Phase 1A PREPARE after locked trusted validation |
|---|---|---|
| PREPARE-normalizable | `AWAITING_CONFIRMATION`, `CONFIRMED` | May apply the already-frozen TTL normalization |
| Legacy execution-authority | `EXECUTING`, `UNKNOWN_OUTCOME`, `RECONCILING` | Keep occupying; return `RETURNED_IN_FLIGHT`; **no** TTL normalize; **no** replacement |

User-facing confirmation TTL does **not** prove that legacy execution activity has been quiesced.

Phase 1A PREPARE normalization and Phase 2A migration normalization are **different authorities** and must not be conflated.

Legacy unresolved states may be normalized only after the hard Phase 2A cutover gate.

### MEDIUM — Non-terminal action plus committed business result HALTs

`AWAITING_CONFIRMATION` or `CONFIRMED` plus a committed source-linked or same-`business_request_key` leave result is contradictory historical evidence.

**HALT migration for explicit adjudication.** Do not auto-normalize either pair to `SUCCEEDED`. Do not hide the contradiction through adoption.

---

## Freeze 2.1 blockers (still in force)

Targeted correctness review of Freeze 2.0 returned **C. FAIL**. The gaps were migration, locking, TTL, and conflict-handling details. They are not a reversal of the simplified core.
...
This Freeze 2.2 commit is documentation only. Evaluation subject / transport fingerprints must remain unchanged from a code perspective.
...
- `AWAITING_CONFIRMATION` + source-linked committed leave result
- `AWAITING_CONFIRMATION` + same-`business_request_key` committed leave result
- `CONFIRMED` + source-linked committed leave result
- `CONFIRMED` + same-`business_request_key` committed leave result
...
The scan must check **both** authoritative leave identities:

- `source_action_id` relationship
- `business_request_key` relationship

and verify canonical trusted equivalence. If either identity indicates a committed business result associated with an `AWAITING_CONFIRMATION` or `CONFIRMED` action: **HALT**.

Do not rely only on workflow state. Do not hide the contradiction through adoption. Do not automatically normalize either pair to `SUCCEEDED`.

`AWAITING_CONFIRMATION` means confirmation had not authoritatively completed. `CONFIRMED` means execution had not authoritatively finalized in the target state model. A committed leave result attached to either is contradictory historical evidence requiring explicit adjudication.
...
Do **not** put a generic “if expired → EXPIRED” step before state-class classification.

### Exact loser ordered steps
...
11. **FIRST classify state class** (after locked trusted validation). Do not read TTL for disposition until this classification is done.

12. **If no occupying row remains:** commit/close T2 and retry `INSERT` within the same bounded PREPARE request.

13. **If Phase 1A and state is a legacy execution-authority state** (`EXECUTING`, `UNKNOWN_OUTCOME`, `RECONCILING`):
    - keep the row occupying;
    - return the existing authoritative action as transitional `RETURNED_IN_FLIGHT` (or the exact existing safe transitional disposition);
    - **STOP** loser normalization;
    - do **not** apply TTL normalization;
    - do **not** transition to `EXPIRED`;
    - do **not** release occupancy;
    - do **not** create a replacement;
    - do **not** treat apparent confirmation expiry as proof that old execution authority is gone.
    - This rule applies **regardless of the apparent TTL** while legacy workers / execution authority may still exist.

14. **Else if state is PREPARE-normalizable** (`AWAITING_CONFIRMATION` or `CONFIRMED`):
    - use the PostgreSQL clock **after** the row lock (`clock_timestamp()`);
    - if expired: transition to `EXPIRED`, supersede/expire any relevant active challenge, append audit, `COMMIT`, then retry `INSERT` within the same bounded PREPARE request;
    - if still valid: return `REUSED_EXISTING`.

15. **Else if `SUCCEEDED`:** return `RETURNED_SUCCEEDED`.

16. Bound PREPARE contention retries.
...
### Phase 1A vs Phase 2A — different authorities

| Authority | Who | May TTL-normalize `AWAITING` / `CONFIRMED` | May normalize `EXECUTING` / `UNKNOWN_OUTCOME` / `RECONCILING` |
|---|---|---|---|
| Phase 1A PREPARE loser | live PREPARE path, old executor may still run | Yes, after locked trusted validation | **No** |
| Phase 2A migration | cutover only, after hard quiescence gate | Yes, as already frozen | **Yes**, using §20 rules only |

The transitional six-state occupancy index remains in force through Phase 1A. A legacy unresolved action cannot temporarily leave occupancy before Phase 2A cutover. The final three-state index remains prohibited before worker quiescence + old-state normalization + clean invariant checks.
...
This section is **Phase 2A migration normalization only**. It is not Phase 1A PREPARE authority.

Legacy unresolved states (`EXECUTING`, `UNKNOWN_OUTCOME`, `RECONCILING`) may be normalized **only after** the hard Phase 2A cutover gate has completed:

- old workers stopped
- restart prevented
- old scheduling stopped
- in-flight legacy execution transactions resolved or terminated
- no old execution authority remains
- authoritative DB / business state inspected
...
A **legacy unresolved** action (`EXECUTING`, `UNKNOWN_OUTCOME`, `RECONCILING`) may normalize to `SUCCEEDED` only after the Phase 2A gate and full identity / business equivalence verification (§13).

Do **not** automatically normalize `AWAITING_CONFIRMATION` + committed leave, or `CONFIRMED` + committed leave, to `SUCCEEDED`. Those pairs HALT (§7).

### Contradictory pairs — HALT

Any of the following must **HALT migration** for explicit adjudication. Do **not** silently auto-correct contradictory terminal or non-terminal history:

- `AWAITING_CONFIRMATION` + source-linked committed leave
- `AWAITING_CONFIRMATION` + same-`business_request_key` committed leave
- `CONFIRMED` + source-linked committed leave
- `CONFIRMED` + same-`business_request_key` committed leave
...
| 28 | Phase 1A PREPARE of `EXECUTING` / `UNKNOWN_OUTCOME` / `RECONCILING` even if TTL appears expired | `RETURNED_IN_FLIGHT`; no `EXPIRED`; occupancy retained |
| 29 | `AWAITING_CONFIRMATION` or `CONFIRMED` plus committed source-linked or same-key leave | HALT migration; no auto-`SUCCEEDED` |
...
- invariant / duplicate preflight (§7); HALT on ambiguity, including non-terminal action + committed leave
- normalize immediately safe expired **PREPARE-normalizable** rows (`AWAITING_CONFIRMATION`, `CONFIRMED`) where appropriate
- do **not** TTL-normalize or release occupancy of `EXECUTING` / `UNKNOWN_OUTCOME` / `RECONCILING`
...
- PREPARE loser `FOR UPDATE` / verification; TTL expiry normalization **only** for `AWAITING_CONFIRMATION` / `CONFIRMED`
- legacy unresolved PREPARE disposition: `RETURNED_IN_FLIGHT`; no replacement
...
| Item | Rule |
|---|---|
| This Freeze 2.2 commit | Documentation only. Code fingerprints unchanged. |
...
Implementation of Freeze 2.2 may begin only after this freeze is committed, **one final two-delta verification** is complete, and Project Controller explicitly authorizes implementation.

Phase 1A remains **NOT AUTHORIZED** by this documentation task.
